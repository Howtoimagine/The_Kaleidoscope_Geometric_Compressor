...
def _header(regime: int, truth: int, debt: DebtRecord, flags: int = 0) -> bytes:
    ...
    # flags gets 2 bits (0-3), which is exactly FLAG_SHELL_PRIOR |
    # FLAG_Z_PRIOR's range - no bits wasted, none missing.
    codeword = encode_12_to_24(_pack_control(VERSION, regime, truth, flags))
    return (
        MAGIC
...
def _parse_header(data: bytes) -> Tuple[int, int, int, DebtRecord, int]:
    ...
    version, regime, truth, flags = _unpack_control(msg)
    if version != VERSION:
...
def compress_tensor(
    ...
    * scale -> 24-D blocks -> exact Leech CVP -> entropy-coded
    ...
    of the theta series, independent of which transform is selected):
